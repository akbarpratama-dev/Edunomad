import { CalendarDays, CalendarClock, Building2, Flag, Users } from "lucide-react";
...
// A project's UMKM / mentor: a small label, then the avatar and clickable name
// (linking to `/users/:id`) so a junior can look them up before applying. When
// there's no user (mentor not yet assigned) the name is a plain muted placeholder.
function PersonRow({
  label,
  name,
  userId,
      ...
      <div className="flex items-center gap-2.5">
        {userId ? (
          <>
            <UserAvatar
              name={name}
              className="size-10 shrink-0 bg-[#d8f277] text-xs font-bold text-[#0b0b0b]"
            />
            <ProfileLink userId={userId} className="truncate text-sm font-semibold text-foreground">
              {name}
            </ProfileLink>
          </>
        ) : (
      ...
      <Card className="app-reveal">
        <CardContent className="flex flex-col gap-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <PersonRow label="UMKM" name={project.umkm.name} userId={project.umkm.id} />
            <PersonRow
              label="Mentor"
              name={project.senior?.name ?? "Belum ada mentor"}
