...
- **12.4 Attachments (NEXT)**: tabel `discussion_attachments` (message_id, type FILE|IMAGE|LINK, url, file_path, file_name, file_size) + Supabase Storage bucket + signed upload + RLS; simpan URL/path saja. Override "no attachments MVP" (docs sudah diamandemen). Composer toolbar attach/image jadi fungsional (ganti placeholder "Segera hadir").
