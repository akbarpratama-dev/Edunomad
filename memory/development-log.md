...
## 2026-07-11b — DEMO prep lanjutan: enrich profil semua akun (skill/experience/portfolio)
User: "isi skill, profile dll di semua role agar realistis; akun demo masih kosong". NO migration. `backend/src/prisma/seed-profiles.ts` (baru, idempotent, di-exclude dari build):
- 3 akun demo (umkm/senior/junior.demo) di-hand-craft PENUH: headline/bio realistis (buang teks "akun demo skenario"), phone, linkedin_url, photo (ui-avatars.com PNG), skills+level, experiences, portfolio_links. senior.demo 6 skill/2 exp/3 port; junior.demo 5/2/2; umkm.demo bio kopi + IG link (UMKM tak perlu skill/exp).
- Semua SENIOR(11)+BEGINNER(7) lain: pastikan skill (bila 0), 2 experience (pool ExpSpec dirotasi per index biar variatif), portfolio (GitHub/LinkedIn/Website atau GitHub/Portfolio), + phone/linkedin bila kosong. TIDAK menimpa headline/bio seed yg sudah ada (setProfile fill-if-empty utk non-demo, overwrite utk demo). Idempotent: experiences/portfolio hanya ditambah bila count=0; skill hanya nama yg belum ada.
- UMKM(13) lain: hanya diisi phone+linkedin company (skill/exp/portfolio dikosongkan — realistis utk bisnis).
Enum dipatuhi: level BEGINNER|INTERMEDIATE|ADVANCED; portfolio type GITHUB|FIGMA|BEHANCE|LINKEDIN|OTHER (dari validator). VERIFIED via Supabase: BEGINNER+SENIOR 0 yg kosong skill/exp/port/phone; 3 akun demo detail lengkap. Profil BERTAHAN lintas demo-reset (reset hanya hapus proyek+notif+aiInsight, bukan user_profiles/skills/experiences/portfolio). CATATAN: bila DB pernah di-reset penuh (reset-expo) akun demo dibuat ulang tanpa profil kaya → jalankan `seed-profiles.ts` lagi.

## 2026-07-12 — Foto proyek asli bertema (ganti placeholder gradient)
User: proyek pakai foto nyata sesuai bisnis (Kopi Nusantara→kopi/biji kopi, Laundry→laundry, dll) biar terasa real. 11 foto Unsplash (tanpa watermark, 800×450, ~40–134KB) diunduh SEKALI ke `frontend/public/projects/<tema>.jpg` (kopi/laundry/roti/salon/klinik/bunga/distro/bangunan/warung/bengkel/bimbel) — lokal = andal saat expo, tak butuh next.config remotePatterns (path lokal). Sumber: Unsplash CDN `images.unsplash.com/photo-<id>` (ID diverifikasi curl+spot-check render: kopi=biji kopi, bengkel=motor, warung=makanan berkuah, bangunan=bor — semua on-theme). LoremFlickr sempat dicoba tapi ada watermark → dibuang. `backend/src/prisma/seed-project-images.ts` (baru, idempotent, FORCE=1 utk overwrite, di-exclude build): map keyword nama UMKM→tema→set `projects.image_url=/projects/<tema>.jpg`. Dijalankan: 22 proyek terisi (1 Toko Maju sudah punya gambar kasir, dilewati). demo.service `createDemoProject` + `DEMO_PROJECT_IMAGE` map (umkm email→foto) + seed/reset-demo-scenario flagship set `imageUrl:/projects/kopi.jpg` → reset tetap bawa gambar. tsc backend build 0. Foto di public/ → Vercel serve statis; frontend Thumb sudah render image_url (fallback gradient bila null). Frontend murni + data → Vercel auto-deploy; foto ikut ter-commit.

## 2026-07-14 — FIX sync junior setelah diterima mentor (bug uji 2 laptop)
Laporan user (uji 2 laptop): junior apply → senior terima → UI junior tak update; "Proyek Saya" kosong padahal sudah diterima; Cari Proyek tampil "masuk" tapi Profil masih "lamaran" → tidak sinkron. Diagnosa (DB junior.demo KONSISTEN: app ACCEPTED + member ACTIVE, proyek masih RECRUITING krn senior belum "Mulai Proyek"). Dua akar: (1) `BeginnerProjectBoard` hanya menampilkan membership dgn `project.status===ACTIVE/AWAITING_COMPLETION` → proyek RECRUITING yg sudah diterima tak muncul; (2) fetch cuma sekali di mount, tanpa realtime/refetch → laptop junior tak tahu senior bertindak. FIX (frontend, no backend/DB): `BeginnerProjectBoard.tsx` — ekstrak `load` useCallback + **refetch on window focus/visibilitychange** (pola D-UI-BADGE-1); tambah state `awaiting` = membership ACTIVE di proyek RECRUITING → empty-state render kartu "Kamu Sudah Diterima! 🎉 menunggu mentor memulai" (bukan render board penuh, hindari label salah). `app/projects/page.tsx` (Cari Proyek) — ekstrak `loadMine` + refetch-on-focus (list + membership markers). tsc 0. Efek: begitu laptop junior di-focus, data ke-refresh; setelah "Terima" langsung muncul kartu diterima; setelah "Mulai Proyek" muncul board penuh. Catatan: Profil (tab Proyek) belum diberi focus-refetch (dipakai fresh di langkah akhir Sertifikat) — bisa ditambah bila perlu. Frontend murni → Vercel auto-deploy.

## 2026-07-12 — Foto asli di landing page (showcase + portfolio)
Lanjutan: terapkan foto nyata ke landing. `landing/sections/project-showcase.tsx` 3 kartu proyek ganti `grad` linear-gradient → `img` foto (`bg-cover bg-center` backgroundImage): kasir (`/projects/kasir.jpg` — download Unsplash baru, kasir+POS+pelanggan), KopiNusantara→kopi.jpg, pemesanan makanan→warung.jpg. `landing/sections/portfolio.tsx` cover sertifikat (h-28 gradient)→`/projects/distro.jpg` (e-commerce) + badge "E" mini→`<LogoMark tone=navy>`. tsc 0. Hero floating cards & avatar gradient dibiarkan (mockup UI dekoratif, bukan foto). Frontend murni → Vercel auto-deploy.

## 2026-07-12 — Brand: logo/icon EduNomad ke seluruh sistem
Aset baru `brand/` (edunomad-icon.svg, edunomad-logo.svg + -dark, preview.html): mark = kotak membulat + huruf "E" tersusun tangga naik (growth/nomad journey), palet lime `#d8f277` + navy `#201f31`. Komponen reusable `frontend/src/components/common/Logo.tsx` → `LogoMark({tone:"navy"|"lime", className})` (navy=badge navy+E lime utk latar terang; lime=badge lime+E navy utk latar gelap; SVG, aria-hidden, shrink-0). Menggantikan treatment CSS huruf "E" lama di: landing **header** (navy), **footer** (lime), **Sidebar** app (lime), **LegalShell** help/privacy/terms (navy), **VerifyView** halaman verifikasi publik (navy, dulu ikon Award), **demo-reset** (navy, dulu RotateCcw). **AuthCard** (login/register) DITAMBAH lockup logo+wordmark (sebelumnya tanpa logo). Favicon: `app/icon.svg` baru (konvensi Next.js App Router, dikonfirmasi Context7) + `app/favicon.ico` default Next DIHAPUS. tsc frontend 0. Perubahan frontend murni → Vercel auto-deploy.

## 2026-07-11c — Cari Proyek: pin flagship demo sbg "Proyek Pilihan"
`frontend/src/app/projects/page.tsx`: helper `isFeaturedDemo(p)` (title mulai "[DEMO]") → FeaturedCard sekarang selalu menampilkan proyek flagship (bila ada di page-1 view), fallback ke view[0] (terbaru) bila tak ada. Badge jadi "Proyek Pilihan · Mulai di sini" saat proyek demo. gridItems exclude by id (bukan slice[1]) biar tak dobel. Tujuan: tester/skenario langsung mendarat di proyek yg benar. Catatan filter existing: BEGINNER list hasSenior:true (flagship muncul setelah senior jadi mentor), SENIOR hasSenior:false (flagship tampil sejak awal) — cocok urutan skenario senior-duluan. tsc frontend 0.
