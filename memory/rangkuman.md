...
[2026-07-11] User (demo prep expo): "coba UMKM yg buka lowongan belum di-apply, hapus junior yg tidak penting" → lalu "bikin akun demo senior+junior utk skenario awal→akhir, 1 proyek unggulan yg dari awal belum ada mentor & junior, semua demo diarahkan kesana" → "lanjut + commit + bikin button/fitur reset". Hasil: (1) Hapus 12 akun filler junior01–12@edunomad.com (Prisma+Supabase Auth, cascade lamaran spam) → 8 lowongan RECRUITING bermentor balik 0 pelamar (skrip `cleanup-juniors.ts`). Junior nama-asli (akbar/dimas/fajar/bima/citra/doni) dipertahankan. (2) Proyek flagship demo `[DEMO] Website Pemesanan & Kasir — Kopi Nusantara` (RECRUITING, no mentor, no member, 1 role FE kap1) + 3 akun demo `umkm.demo`/`senior.demo`/`junior.demo@edunomad.com` (pw TestPass123!). (3) Fitur reset: didaftarkan ke `demoDataset.ts` (source of truth) → endpoint baru `POST /demo/reset-scenario` (scoped, token+DEMO_MODE gated) + `resetDemoScenario()` di demo.service + tombol "Reset Proyek Skenario" (primary) di `/demo-reset` (tombol lama jadi "Reset Semua Data Demo" secondary) + CLI `reset-demo-scenario.ts`. Skrip seed `seed-demo-scenario.ts`. tsconfig.build exclude semua skrip dev prisma. VERIFIED: tsc backend(build)+frontend 0; CLI reset jalan (hapus+buat ulang, state RECRUITING/no-mentor/no-member/0 pelamar terverifikasi via Supabase); 3 akun VERIFIED. Decision D-DEMO-2. Alur demo: senior lamar mentor→(DEMO_MODE auto-accept / umkm.demo terima)→junior lamar→senior terima→Mulai→...→Selesaikan (D-P14-1)→sertifikat.

[2026-07-11] User (lanjutan, expo 2 hari lagi): "isi skill, profile dll di semua role agar realistis; akun demo masih kosong". Hasil: `seed-profiles.ts` (idempotent) — 3 akun demo di-hand-craft penuh (headline/bio realistis, phone, linkedin, photo ui-avatars, skills+level, experiences, portfolio) + semua SENIOR(11)/BEGINNER(7) diberi skill (bila kurang)+2 experience variatif+portfolio+phone/linkedin (fill-if-empty, tak menimpa bio seed). UMKM diisi phone/linkedin saja. VERIFIED via Supabase: 0 SENIOR/BEGINNER kosong skill/exp/port/phone. Profil bertahan lintas demo-reset. Commit lanjutan.
