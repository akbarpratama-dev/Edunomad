...
User dapat tugas "implementasi penggunaan AI". Via AskUserQuestion dipilih: provider **Google Gemini** (`gemini-2.0-flash`, API key gratis AI Studio); pendekatan **LLM + structured JSON + Explainable** (bukan embeddings/pgvector); lingkup **3 fitur sekaligus**; data "minat" **skip** (pakai data existing). Branch `feature/ai-matching` (dari main setelah commit portofolio ece0e95). Plan disetujui (plan mode).
3 FITUR: (1) **AI Candidate Matching & Ranking** `GET /projects/:id/applicants/ranking` (SENIOR lead, ownership di service) — skor 0-100 + matchedSkills/missingSkills + alasan per pelamar PENDING, 1 panggilan batch. (2) **AI Portfolio Recommendation** `GET /projects/:id/portfolio-recommendation` (BEGINNER own, requireVerified) — item porto/cert/experience yg relevan utk lowongan + highlights + advice. (3) **AI Professional Summary** `GET /users/me/ai-summary` + `GET /users/:id/ai-summary` — ringkasan + strengths + suggestedRoles (skip UMKM).
ARSITEKTUR: `config/env.ts` +geminiApiKey/geminiModel/aiEnabled (SOFT, bukan required — app tetap boot tanpa key). `services/gemini.service.ts` (transport: fetch v1beta generateContent, header x-goog-api-key, responseMimeType application/json, AbortController 12s; semua gagal → `AiUnavailableError`). `services/aiInsight.service.ts` (3 method + wrapper `cachedInsight`: sha256(inputs) → findFresh → !enabled? unavailable → generateJson → zod validate/clamp → upsert; AiUnavailableError → {available:false}). `repositories/aiInsight.repository.ts` (findFirst + find-then-update/create; Prisma compound-unique tak terima null projectId). Tabel `ai_insights` (kind VARCHAR+CHECK, subject_user_id, project_id nullable, input_hash, result_json jsonb, model; UNIQUE (kind,subject_user_id,project_id) NULLS NOT DISTINCT — PG17.6). Migration `20260706160000_ai_insights` LIVE (apply_migration) + _prisma_migrations synced (checksum b8832b5a…) + prisma generate. Controller `modules/ai/ai.controller.ts`, validator `validators/ai.validator.ts` (regenerate query), routes di project.routes + user.routes. Envelope `AiResult<T>` = {available:true,cached,generatedAt,data} | {available:false,reason} — SELALU HTTP 200.
FRONTEND: `services/aiApi.ts` + komponen `ai/{AiUnavailable,MatchScoreBadge,AiSummaryCard,PortfolioRecPanel}.tsx`. Wiring: applicants page (fetch ranking terpisah/paralel — tak blokir daftar; toggle "Urutkan berdasarkan kecocokan AI"; per kartu PENDING: MatchScoreBadge + blok Analisis AI matched/missing/reason; tombol Perbarui peringkat; fallback banner), project detail apply dialog (PortfolioRecPanel enabled saat dialog buka), ProfileView (AiSummaryCard non-UMKM). Semua punya skeleton + fallback AiUnavailable ("Coba lagi").
RULES DIJAGA: TANPA embeddings/pgvector, TANPA service/queue/background job baru (modular monolith), 1 tabel, fetch sinkron dalam request. Kontrol biaya: caching + truncation (bio/desc 500, artifacts/exp 10, motivasi 300) + maxOutputTokens 1024 + on-demand. GEMINI_API_KEY di backend/.env MANUAL (sandboxed). Output Bahasa Indonesia.
VERIFY (fallback): backend+frontend tsc 0. curl no-key → available:false; RBAC beginner→ranking 403, senior→portfolio-rec 403; Playwright /profile fallback mulus.

UPDATE (2026-07-06, HAPPY PATH VERIFIED + provider multi):
- Gemini free-tier quota MODEL-SPECIFIC: `gemini-2.0-flash` = 429 "exceeded quota" (kuota 0 utk akun user; region Indonesia SUPPORTED, jadi bukan region — konfirmasi via ai.google.dev). Default diganti → **`gemini-2.5-flash`** = JALAN. Key format baru Google = "auth key" (diawali `AQ.`, bukan `AIzaSy` — valid, aturan baru per ai.google.dev/gemini-api/docs/api-key). Gemini 2.5 "thinking" makan output-token → JSON truncated → matikan `thinkingConfig.thinkingBudget:0` + `maxOutputTokens 2048` + strip markdown fence.
- Multi-provider: `gemini.service.ts` → `services/llm.service.ts` (Groq preferred bila GROQ_API_KEY, else Gemini). env += groqApiKey/groqModel (`llama-3.3-70b-versatile`). Groq OpenAI-compatible (Context7-checked) sbg fallback global bebas dinding-kuota. aiInsight.service pakai llmService + activeModel(). Server-side `console.warn("[AI] …")` utk diagnosa.
- HAPPY PATH VERIFIED (Gemini gemini-2.5-flash, output nyata Bahasa Indonesia): (1) ai-summary → ringkasan+strengths+suggestedRoles; (2) portfolio-recommendation → recommendedItems+highlights+advice; (3) ranking (2 pelamar PENDING demo di …0005) → Beginner Two **85%** matched[JS,React] missing[TS] vs Beginner Three **30%** missing[JS,React,TS], Explainable. Cache: 2nd call cached:true. Playwright applicants page: badge %+blok Analisis AI+toggle sort+Perbarui OK. Demo data DB: role Frontend …051 += required JS/React/TS; B2/B3 user_skills + 2 project_applications PENDING (cleanup nanti kalau mau DB bersih).
...
### D-AUTH-1 (2026-06-30, branch feature/phase-10-discussion-forum): fix race redirect ke /auth/login
Gejala: user login lalu tiap akses page lain dilempar ke /auth/login. Akar masalah = race di client-side guard, BUKAN session/persist/backend. Context7 supabase-js: `signInWithPassword` me-resolve promise DAN dispatch event SIGNED_IN secara terpisah; pola resmi adalah navigate DI DALAM callback onAuthStateChange. Login page kita `router.push("/dashboard")` tepat setelah promise resolve → kadang event SIGNED_IN (yg meng-update zustand store) sampai SETELAH dashboard mount → AuthGuard lihat isLoading=false (sudah false dari boot tanpa session) + isAuthenticated=false → `router.replace("/auth/login")`. Race kedua: isAuthenticated flip true sebelum fetchMe() selesai (appUser null) di client-nav → lempar /auth/register/role.
KEPUTUSAN: perbaiki di guard, bukan di tiap call-site (lebih tahan banting: juga handle token-refresh & multi-tab):
1. AuthGuard.tsx — sebelum `router.replace("/auth/login")`, panggil `supabase.auth.getSession()`; hanya redirect jika session BENAR-BENAR null. Jika ada (store cuma telat), jangan redirect — onAuthStateChange akan catch up & guard re-render ke children. Render gate lama (`Memuat…` saat !isAuthenticated) tetap → tak ada flash login.
2. AuthProvider.tsx — pada session event, jika appUser belum ada (fresh login/initial), `setLoading(true)` selama fetch app-user supaya guard nunggu (cegah race #2 lempar ke register/role). TOKEN_REFRESHED (appUser sudah ada) = refetch silent, tanpa flash "Memuat…". setLoading(false) re-guard `!active`.
3. useRequireSession.ts — getSession re-cek sama sebelum redirect (step registrasi 2-5).
Alternatif ditolak: navigate di dalam onAuthStateChange (harus ubah banyak call-site + sulit per-page target); set session manual di login page (cuma nutup race #1, bukan #2/refresh). tsc 0. BELUM E2E browser (butuh backend+creds) — perlu verifikasi sesi berikut.
CATATAN: nav/dashboard/login masih link ke page 404 (/artifacts /notifications /help /privacy /terms /auth/forgot-password) — keputusan stub vs hapus diserahkan ke user.

### D-P12-2 (2026-07-01, branch feature/phase-10-discussion-forum): sidebar "Proyek Mentoring" utk SENIOR (akses diskusi simetris)
Masalah: user merasa role mentor "tak punya discussion board" seperti mahasiswa. Realita: diskusi = tab di workspace proyek, identik utk semua role; per docs/08 sidebar resmi (Dashboard/Projects/Artifacts/Notifications) TIDAK punya page Diskusi mandiri (group discussion per-proyek, anggota saja). Akar asimetri = SENIOR tak punya entri sidebar menuju proyek mentoring-nya (BEGINNER punya "Proyek Saya", UMKM punya "Proyek Saya"); /my-projects di-guard UMKM+BEGINNER. Plus kartu "Diskusi Baru" di dashboard beginner cuma sample → memperkuat kesan beda.
KEPUTUSAN (user pilih Opsi 1, tetap patuh docs — diskusi TIDAK dijadikan page sidebar mandiri): tambah nav SENIOR "Proyek Mentoring" → reuse /my-projects (RoleRouter + SeniorView baru pakai projectApi.mentoredProjects() yg sudah ada), guard +SENIOR. Jalur jadi simetris: Proyek Mentoring → Buka Workspace (ACTIVE/AWAITING) → tab Diskusi (board sama dgn mahasiswa). Opsi ditolak: page /discussions agregat lintas-proyek (navigasi baru = override docs, tak dipilih user). Files: constants/navigation.ts, app/my-projects/page.tsx. tsc 0; browser-verified p4-senior end-to-end (sidebar→list→workspace→Diskusi render penuh). UNCOMMITTED.

### D-P12-3 (2026-07-01, branch feature/phase-10-discussion-forum): nested route /my-projects/:id (+workspace) — flow stay-in-namespace
User: dari my-projects, buka proyek (detail/diskusi) harus tetap di URL /my-projects/... (contoh /my-projects/:id/workspace?tab=discussion). Jelajahi Proyek = discovery only (/projects/:id utk lamar).
KEPUTUSAN: TIDAK duplikat komponen. Detail + workspace page jadi base-aware (usePathname → base /my-projects vs /projects) utk backHref + internal link. Route nested baru hanya re-export default komponen yg sama (app/my-projects/[id]/page.tsx + app/my-projects/[id]/workspace/page.tsx). workspace deep-link tab via ?tab= (baca window.location.search, pola register, hindari Suspense). Kartu my-projects repoint ke /my-projects/:id*. /projects/:id tetap utk browse.
Catatan deviasi docs/08: URL workspace docs = /projects/:id/workspace; sekarang ADA juga /my-projects/:id/workspace (komponen sama, bukan screen/flow baru — masih workspace per docs, hanya namespace entri). User-driven, dalam semangat sama D-P12-2.
SISA: detail "Kelola"/"applicants" masih /projects/:id/manage|applicants (mgmt route belum di-nest); dashboards + BeginnerProjectBoard masih link /projects/:id/workspace (belum migrasi — bisa dipindah kalau user mau full konsisten). tsc 0; browser-verified p4-senior. UNCOMMITTED.

### D-P12-4 (2026-07-01, branch feature/phase-10-discussion-forum): Phase 12.2 Threaded Replies (one level)
discussion_messages.parent_id self FK (ON DELETE CASCADE). Balasan HANYA satu tingkat: reply ke top-level saja; reply-ke-reply ditolak (service cek parent.parentId==null → BusinessRuleError "Balasan hanya satu tingkat"; parent harus same discussion → NotFound). listMessages kini top-level only (parentId null) + include replies (asc); countMessages top-level only → header "X pesan" hitung topik, bukan total+replies (_count.messages discussion tetap all — minor, dipakai di kartu list). sendMessageSchema parentId optional dipakai bersama group+DM tapi DM controller tak pass (abaikan). Realtime cukup existing filter discussion_id (replies INSERT tetap men-trigger re-pull). Frontend: Bubble reusable (compact utk reply), composer inline per-pesan via state replyTo tunggal. Migration via Supabase MCP + _prisma_migrations manual (checksum=sha256 file). tsc/build 0; E2E browser OK.

### D-P12-5 (2026-07-01, branch feature/phase-10-discussion-forum): Phase 12.3 Reactions
Tabel message_reactions; unique(message_id,user_id,emoji); toggle (add absent / remove present). Emoji = allowlist kurasi (👍❤️🎉✅👀🙌), divalidasi Zod enum server-side (bukan free-form). KEPUTUSAN denormal: simpan discussion_id di message_reactions (turunan dari message) supaya bisa reuse is_discussion_member(discussion_id) utk RLS SELECT + filter Realtime per-discussion (filter Realtime hanya eq, tak bisa via join). Trade-off normalize→denormalize diterima demi realtime/RLS (sejajar pola discussion_messages). Prisma: MessageReaction hanya relasi `message` (userId/discussionId scalar, no back-relation) utk hindari lebarkan model User/Discussion; FK tetap ada di DB. listMessages include reactions {emoji,userId} di message+replies → frontend group (count + mine). Route POST /discussions/messages/:messageId/reactions (4 segmen, tak bentrok /:id/messages). Realtime: publication += message_reactions, RLS select policy, replica identity full; feed listen event * filter discussion_id. tsc/build 0; E2E browser add+remove OK.

### D-P12-6 (2026-07-01, branch feature/phase-10-discussion-forum): Phase 12.4 Attachments (Supabase Storage)
Override locked "no attachments MVP" (docs/03,06 sudah diamandemen sejak 12.1). Bucket PRIVAT `discussion-attachments` (10MB). Flow: client minta signed upload URL ke backend (POST /discussions/:id/attachments/upload-url, member+verified) → upload langsung ke Storage via uploadToSignedUrl → kirim message dgn attachments[] metadata (type, filePath/url, fileName, fileSize) → backend createMessage nested-create attachments. Baca: backend tanda-tangani signed download URL (createSignedUrl TTL 1h, service role) per attachment FILE/IMAGE saat listMessages/echo; LINK pakai url tersimpan. FILE STORAGE rule dipatuhi (DB simpan path/url+metadata saja, biner di Storage). KEPUTUSAN: akses Storage SEMUA via signed URL (admin) → TIDAK perlu storage.objects RLS; tabel discussion_attachments TIDAK perlu RLS/publication (dibaca via Express, bukan client PostgREST; realtime message INSERT existing sudah men-trigger re-pull lampiran). Prisma DiscussionAttachment relasi `message` saja. sendMessageSchema: message optional + refine "pesan ATAU lampiran" (DM tetap wajib message krn tak kirim attachments). Frontend: composer Paperclip=file apa saja, ImageIcon=accept image; staged pending sebelum kirim; IMAGE render <img signed>, lainnya chip link. Allowlist size ≤10MB di Zod + bucket. LINK type didukung backend; UI tautan khusus belum (defer, FILE+IMAGE cukup utk "toolbar attach/image fungsional"). Context7 utk API storage. tsc/build 0; E2E upload .txt OK.

### D-P12-7 (2026-07-01, branch feature/phase-10-discussion-forum): Phase 12.5 Views (PHASE 12 selesai)
Tabel discussion_views, unique(discussion_id,user_id) = unique-viewer count. recordView = upsert idempotent (buka topik berkali-kali tetap 1 view/user). Count via _count.views di list discussion (bukan query terpisah). Route POST /discussions/:id/view = authMiddleware saja TANPA requireVerified (view sisi-baca, sejajar listMessages). Tanpa RLS/publication (dibaca via Express). Frontend: DiscussionTab effect recordView(activeId)→load() saat topik dibuka (refresh count); header feed "{n} pesan · {n} dilihat". recordView best-effort (.catch undefined) supaya gagal view tak ganggu UX. tsc/build 0; E2E "1 dilihat" OK.
PHASE 12 (Discussion Forum Upgrade) SELESAI: 12.1 title+category+pin (D-P12-1) · 12.2 threaded replies (D-P12-4) · 12.3 reactions (D-P12-5) · 12.4 attachments (D-P12-6) · 12.5 views (D-P12-7). Siap merge ke main.

### D-UI-12 (2026-07-01, branch feature/phase-10-discussion-forum): unify Button radius ke tema card
User minta semua button 1 tema (contoh radius samakan dgn card) pakai skill impeccable/emil-design-eng/web-design-guidelines. Masalah: Button `default`+`outline` = rounded-full (pill) sedangkan `secondary/ghost/destructive` = rounded-lg(8) → tak konsisten; sementara card=20px, input=10px. KEPUTUSAN: skala radius koheren satu keluarga — base Button rounded-lg→rounded-2xl(16px) (di antara input 10 & card 20), buang rounded-full dari default+outline; size kecil tetap proporsional (sm=12, xs/icon-xs=10). Chips/FilterChip/Badge TETAP pill (kelas tag, konvensi beda). Select trigger tetap rounded-lg (form-control, tak diubah—di luar scope "button"). File: components/ui/button.tsx. tsc 0; browser-verified (Lihat Detail Proyek/Reset Filter kini soft-rect sejajar card, bukan pill). Catatan: docs/08 sebut primary rounded-full (pill) → override visual atas permintaan user (UI spec = referensi, boleh ditingkatkan per CLAUDE.md).

### D-UI-13 + D-P12-3b (2026-07-01, branch feature/phase-10-discussion-forum)
(D-UI-13) Select trigger radius rounded-lg(8)→rounded-[10px] = sekeluarga input (control family 10), serasi dgn button 16/card 20; tak pakai 16 di trigger h-8 krn jadi pill. File select.tsx.
(D-P12-3b) Workspace/manage/applicants jadikan KANONIK di /my-projects (area kerja member/owner), bukan base-aware lagi: detail ActionPanel "Buka Workspace"/"Kelola Lamaran Senior|Beginner" → hardcoded /my-projects/:id/* (base ActionPanel dihapus, unused); applications/mentor "Kelola Lamaran Beginner" → /my-projects/:id/applicants (sebelumnya /projects). Detail page sendiri tetap base-aware (bisa diakses dari Jelajahi Proyek /projects/:id utk apply, atau /my-projects/:id). Hasil: TIDAK ada lagi link UI ke /projects/:id/workspace (URL itu masih ada via re-export tapi tak dipakai). Verifikasi: detail browse /projects/:id → link workspace/applicants = /my-projects. Catatan: tab browser /projects/.../workspace yg user lihat = sisa tab navigate manual, bukan link app.

### D-UI-14 (2026-07-01, branch feature/phase-10-discussion-forum): refine small-button radii + DESIGN.md + drop card "Lihat Detail"
Gejala user: button di /my-projects "belum berubah" stlh D-UI-12. Sebab: size sm/xs/icon-sm/icon-xs override radius `rounded-[min(var(--radius-md),N)]` dan --radius-md=8px → small button = 8px (kotak), MENANG atas base & tak ikut refine. Fix: base button rounded-2xl→rounded-[16px] eksplisit; size sm/icon-sm→rounded-[12px], xs/icon-xs→rounded-[10px] eksplisit (skala satu keluarga: btn16/sm12/xs10, sejajar input10/card20). Sekarang SEMUA button (semua halaman, krn komponen shared) ikut tema.
...
### D-P10-1 (2026-07-05, main): Phase 10 profile page — Sertifikat tab in-scope, CV out
User approved building the "Sertifikat/Portofolio" TAB inside the profile page showing the user's own VERIFIED certificates (reuses existing Artifact data). Amends D-P8-5: this is the in-app profile tab, NOT the standalone public page `/portfolio/:id` (still deferred). Stats: 3 real (Sertifikat Terverifikasi, Proyek Selesai, Rata-rata Ulasan) — dropped mockup's "Jam Kontribusi" (no schema field). Skill bars mapped from level (BEGINNER/INTERMEDIATE/ADVANCED → 40/70/100%). CV PDF = disabled placeholder (out of scope, no generator). Backend: composite `GET /users/:id/profile-overview` (userRepository aggregates). Frontend: ProfileView (shared /profile + /users/:id), /profile/edit (core fields RHF+Zod), Header dropdown "Profil Saya", UserAvatar gains optional `src`. tsc 0 both; DOM-verified beginner (stats 1/2/4.0×2). commit 6c837c4.

### D-P10-2 (2026-07-05, main = 779902b): profile info fields + role gating
User req: tambah Skill Badge/Top Skill, Total Jam Kontribusi, Total Review, Completed Project, Current Project, Role, Joined Since, Social Links (Github/LinkedIn/Website/Portfolio) — berlaku semua role KECUALI UMKM. Cek data: semua fit kecuali "Total Jam Kontribusi" (ContributionReport TANPA kolom jam). Putusan (AskUserQuestion): (1) Jam Kontribusi = PLACEHOLDER 0 (tanpa migration; stats.contributionHours=0 hard-coded di service, komentar D-P10-2). (2) UMKM = info dasar saja (Role/Joined/Social/Proyek; tanpa stat grid/skills/CV; tab Sertifikat+Ulasan disembunyikan). Impl: backend countCurrentProjects + stats {currentProjects, contributionHours}. Frontend ProfileView: `professional = role!=="UMKM"` gate; hero meta +Role +Top Skill; sidebar 6 stat box (Sertifikat/Selesai/Berjalan/Total Ulasan/Jam 0/Rata-rata) + Top Skill highlight + "Tautan Sosial" card (semua role) + CV placeholder (non-UMKM). tsc 0 both. E2E: beginner 6 stat render (1/2/1/2/0/4.0); UMKM viewed via /users/:id = 2 tab, no stat/edit. Jika nanti mau jam nyata → tambah hours_logged ke contribution_reports + input di form submit.

### D-P10-3 (2026-07-05, main): sisa Phase 10 — static/error/auth/profile-mgmt/admin
Batch penutup Phase 10 (4 wave, 5 commit: 42f352c, ac92828, 79c7565 + docs):
1. Static/error: /help /privacy /terms (LegalShell standalone, perbaiki 404 footer sidebar) + app not-found.tsx (404) + error.tsx (boundary retry).
2. Password reset: /auth/forgot-password (supabase.auth.resetPasswordForEmail, sent-state non-enumerating) + /auth/reset-password (guard recovery-session + updateUser). Perbaiki link login yang menggantung.
3. Profile sub-resource mgmt di /profile/edit: SkillsManager/ExperiencesManager/LinksManager → CRUD /users/me/{skills,experiences,portfolio-links} (sudah ada backend). Skills+Experiences disembunyikan utk UMKM (D-P10-2); Links semua role. Fix base-ui Select.Value tampil enum mentah → render-fn label. E2E: add skill JavaScript persist ke profil.
4. Admin monitoring + replace-senior (Workflow 16, docs/04): GET /admin/projects (semua status + mentor + _count members ACTIVE), GET /admin/projects/:id/senior-candidates (SENIOR VERIFIED, active<5, ≠current), POST /admin/projects/:id/replace-senior (validasi status ACTIVE/AWAITING + role + VERIFIED + kapasitas; setSenior + audit SENIOR_REPLACED + notif SENIOR_ASSIGNED/SENIOR_REMOVED ke dua mentor). Frontend /admin/projects (filter+search+rows+dialog Ganti Mentor) + nav "Pantau Proyek". Notif types +SENIOR_ASSIGNED/SENIOR_REMOVED; audit +SENIOR_REPLACED.
E2E: backend list+candidates+guards(422)+happy-path (via 2nd-senior sementara, di-revert bersih) semua hijau; Playwright admin page+dialog render. tsc 0 backend+frontend. Catatan: hanya 1 senior VERIFIED di data test → dialog "Tidak Ada Kandidat" (benar).

### D-P12-8 (2026-07-07): diskusi grup boleh dibuat oleh junior/beginner (rule change)
User: "ubah aturan, diskusi bisa dibuat oleh junior juga". Membalik aturan Phase 12 lama (create topic = senior lead / UMKM owner ONLY; beginner join/post saja). Aturan BARU: **create group discussion = peserta proyek mana pun** — UMKM owner, assigned senior, ATAU member ACTIVE (termasuk BEGINNER). Harus VERIFIED. **Pin/unpin TETAP** senior lead / UMKM owner only (aksi moderasi, tak berubah). Impl: backend `discussion.service.createGroupDiscussion` — buang cek isSenior/isOwner, ganti jadi "creator harus ada di participantIds (owner+senior+ACTIVE members)". Route sudah cuma auth+requireVerified (no roleMiddleware) → beginner sampai ke service. Frontend `DiscussionTab`: pisah `canManage` (pin, lead/owner) dari `canCreate` (= canManage || member ACTIVE); `canPin={canManage}` (dulu `={canCreate}`). Docs diamandemen: 06-RBAC §Discussion Rules, 07-Workflow (SEND MESSAGES flow), 04-API (Create Group Discussion Access). tsc 0 backend+frontend. Belum di-commit.

### D-P14-1 (2026-07-08): penyelesaian proyek 1-klik oleh Senior + auto-sertifikat (revisi Workflow 11/15)
User: "senior cukup klik selesaikan projek, lalu junior otomatis dapat sertifikat" + (AskUserQuestion) pilih "satu klik, syarat tetap dijaga" untuk mode nyata TAPI "untuk demo cukup satu klik tanpa syarat", dan "hapus konfirmasi UMKM, senior finalisasi sendiri".
Aturan LAMA (Workflow 11 + 15): Senior "Ajukan Penyelesaian" (ACTIVE→AWAITING_COMPLETION) → UMKM "Konfirmasi Penyelesaian" (→COMPLETED); sertifikat digenerate MANUAL per-beginner lebih dulu (Workflow 13) sebagai prasyarat gate.
Aturan BARU: **Senior klik "Selesaikan Proyek" SEKALI → ACTIVE langsung COMPLETED** (buang AWAITING_COMPLETION dari alur utama + buang konfirmasi UMKM). Sertifikat tiap beginner aktif **otomatis diterbitkan** sebagai bagian dari penyelesaian (Workflow 13 dilipat ke penyelesaian; idempotent — skip yg sudah punya). Gate Workflow 15 (deliverable APPROVED + kontribusi + review per-beginner) **TETAP dijaga** kecuali flag demo aktif → tombol gagal dgn daftar blocker bila belum siap. Nilai inti "verified artifact" + "Reviews mandatory before completion" tetap utuh di mode nyata.
DEMO: env **`DEMO_COMPLETE_BYPASS=true`** (soft, default false, di backend/.env yg sandboxed → user isi manual) → skip gate + terbitkan sertifikat dgn data placeholder bila kontribusi/review belum ada (untuk expo cepat). Off = sertifikat tetap didukung kerja nyata.
Impl backend: `artifact.service` ekstrak helper `_issueArtifact` + method baru `generateForCompletion(strict)` (tolerant placeholder saat !strict); `projectLifecycle.service.completeProject` (senior lead, gate opsional-artifact `completionBlockers(_,false)`, panggil generateForCompletion, `completeWithAudit(_, seniorId)`, notif PROJECT_COMPLETED ke UMKM); `env.demoCompleteBypass`; constants/notificationType += PROJECT_COMPLETED; controller `completeProject` (verification_url dari body/host); route POST /projects/:id/complete → completeProject (SENIOR). `requestCompletion`/`confirmCompletion` lama DIPERTAHANKAN utk back-compat proyek yg terlanjur AWAITING_COMPLETION.
Impl frontend: projectApi `complete()` (kirim verification_url origin/verify) + `workspaceSummary()`; detail page panel senior ACTIVE → tombol "Selesaikan Proyek" (dulu "Ajukan Penyelesaian").
VERIFIED E2E (Playwright, senior Aldo, proyek Kasir): tombol render → dialog copy baru → klik → DB: status COMPLETED + completed_at + 2 artifacts auto (EDN-2026-000002/003) + members COMPLETED. tsc 0 backend+frontend. Docs 07 (Workflow 11+15) diamandemen. Belum di-commit.

### D-P14-2 (2026-07-08): badge "butuh perhatian" per tab workspace
Permintaan user: tab workspace kasih notif jumlah supaya senior/junior notice ada perubahan (deliverable baru brp, harus review brp orang, kontribusi yg harus diisi, dll).
Impl: backend `GET /projects/:id/workspace-summary` (auth) → `workspaceSummary.service` role-aware counts {milestones, deliverables, contributions, reviews, artifacts} (senior: deliverable SUBMITTED / kontribusi PENDING / beginner belum di-review / beginner belum bersertifikat / milestone belum COMPLETED; beginner: draft+revision deliverable-ku / 1 bila kontribusi belum submit; umkm: target review tersisa). Controller `modules/workspace/workspaceSummary.controller` + route. Frontend: projectApi.workspaceSummary + type WorkspaceSummary; PillTab += `tone:"alert"` (badge merah bila count>0, sembunyi bila 0); workspace page fetch summary (re-fetch tiap ganti tab) → badge per tab. VERIFIED Playwright: senior Kasir lihat badge "Sertifikat 2" (2 junior belum bersertifikat). tsc 0. Belum di-commit.

### D-DEMO-2 (2026-07-11): akun demo skenario terdedikasi + proyek flagship + reset scoped
User (persiapan expo): "hapus junior yg tidak penting"; "bikin akun demo senior+junior utk skenario awal→akhir, 1 proyek unggulan yg dari awal belum ada mentor & junior — semua demo diarahkan kesana biar tak bingung pilih proyek"; "bikin button/fitur reset".
Keputusan:
1. **Hapus filler junior01–12@edunomad.com** (12 akun, murni pelamar spam dari seed-lowongan: 0 artifact/member/review) via `cleanup-juniors.ts` (Prisma delete + Supabase Auth deleteUser; FK beginner_id CASCADE membersihkan lamaran). Efek samping DISENGAJA: 8 lowongan RECRUITING bermentor balik 0 pelamar = skenario "UMKM buka lowongan belum di-apply" yg bersih. Junior nama-asli (akbar/dimas/fajar/bima/citra/doni, punya artifact/membership) TETAP.
2. **1 proyek flagship** `[DEMO] Website Pemesanan & Kasir — Kopi Nusantara` — RECRUITING, seniorId null (belum ada mentor), tanpa member, 1 role Frontend Developer kap 1. + **3 akun demo terdedikasi** `umkm.demo`/`senior.demo`/`junior.demo@edunomad.com` (pw TestPass123!). Alur: senior lamar mentor → (DEMO_MODE auto-accept ATAU umkm.demo terima manual) → junior lamar → senior terima → Mulai → deliverable/kontribusi/review → Selesaikan (D-P14-1) → sertifikat.
3. **Reset** (dua level, keduanya token+DEMO_MODE gated): flagship didaftarkan ke `demoDataset.ts` (single source of truth: DEMO_SCENARIO_* + masuk DEMO_UMKMS/DEMO_PROJECTS/participant emails). Baru: `resetDemoScenario()` (scoped — hapus proyek umkm.demo + notif/ai 3 peserta → re-seed HANYA flagship) + `POST /demo/reset-scenario` + tombol PRIMARY "Reset Proyek Skenario" di `/demo-reset` (tombol lama `/demo/reset` full-reset jadi secondary "Reset Semua Data Demo"). CLI kembar `reset-demo-scenario.ts`; seed `seed-demo-scenario.ts`. Akun demo tak pernah dihapus reset (persist lintas run).
Impl: demo.service refactor (ensureUser generik role UMKM|SENIOR|BEGINNER + ensureScenarioAccounts + buildSeedContext/createDemoProject helper + resetDemoScenario); demo.controller `guard()` bersama + `resetScenario`; demo.routes +/reset-scenario; frontend demo-reset/page.tsx dua tombol (busy per-kind). tsconfig.build.json exclude semua skrip dev prisma (seed-lowongan/seed-showcase/cleanup-test-data + 3 baru) supaya dist ramping.
VERIFIED: tsc backend(build config)+frontend 0 (error base-tsconfig hanya di seed-ai-demo/seed-expo lama yg memang di-exclude); CLI reset-demo-scenario jalan (hapus+buat ulang, state RECRUITING/no-mentor/no-member/0 pelamar terverifikasi via Supabase); 3 akun VERIFIED. HARD RULE demo dipertahankan: endpoint 404 bila !DEMO_MODE, 401 token salah — tak bisa nyala di produksi.
